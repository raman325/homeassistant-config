...
class Include(yaml.YAMLObject):
    ...
    def __repr__(self):
        return 'Secret({})'.format(self.include)
    ...
    @classmethod
    def to_yaml(cls, dumper, data):
        return dumper.represent_scalar(cls.yaml_tag, data.include)
...
def decode_ha(item):
    if isinstance(item, str):
        if item[0,7] == "!secret":
            return Secret(item[-8:])
        elif item[0,8] == "!include":
            return Include(item[-9:])
        elif item == "":
            return None
    return item
...
# yaml.add_multi_representer(Secret, Secret.to_yaml)
